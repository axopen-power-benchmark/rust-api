use super::canopee_user_model::CanopeeUser;
...
fn test_insertion_deletion() {
    run_test!(|client, conn| {
        ...
        let init_tasks = CanopeeUser::all(&conn).await.unwrap();
        ...
        let new_tasks = CanopeeUser::all(&conn).await.unwrap();
        ...
        // Ensure it's gone.
        let final_tasks = CanopeeUser::all(&conn).await.unwrap();
...
fn test_many_insertions() {
    ...
    run_test!(|client, conn| {
        ...
        let init_num = CanopeeUser::all(&conn).await.unwrap().len();
        ...
        for i in 0..ITER {
            ...
            let tasks = CanopeeUser::all(&conn).await.unwrap();
